//! `faer` provides utilities for computing and manipulating the $QR$
//! factorization with and without pivoting. the $QR$ factorization decomposes a
//! matrix into a product of a unitary matrix $Q$ (represented using block
//! householder sequences), and an upper trapezoidal matrix $R$, such that their
//! product is equal to the original matrix (or a column permutation of it in
//! the case where column pivoting is used)
//!
//! # example
//!
//! assume we have an overdetermined system $Ax = b$ with full rank, and that we
//! wish to find the solution that minimizes the 2-norm
//!
//! this is equivalent to computing a matrix $x$ that minimizes the value $||Ax
//! - b||^2$, which is given by the solution $$x = (A^H A)^{-1} A^H b$$
//!
//! if we compute the $QR$ decomposition of $A$, such that $A = QR =
//! Q_{\text{thin}} R_{\text{thin}}$, then we get $$x = R_{\text{thin}}^{-1}
//! Q_{\text{thin}}^H b$$
//!
...
mod tests {
	...
	fn test_example() {
		...
		let rank = Ord::min(a.nrows(), a.ncols());
		let block_size =
			factor::recommended_block_size::<f64>(a.nrows(), a.ncols());
		...
		for i in 0..rank {
			for j in 0..b.ncols() {
				assert!(
					(solution[(i, j)] - expected_solution[(i, j)]).abs()
						<= 1e-6
				);
			}
		}
